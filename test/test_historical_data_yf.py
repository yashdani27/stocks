...
# final_df = get_data_from_start_date('TATAPOWER', 'NSE', 'weekly', 1, 1, 2021, True, True)
...
ticker = prepare_ticker('TATAPOWER', 'NSE')
arr_date = str(datetime.datetime.now()).split(' ')[0].split('-')
year = int(arr_date[0])
month = int(arr_date[1])
print('month', month)
day = int(arr_date[2])
# period1 = get_start_date(day, (month - 3) if (month - 3) > 0 else (12 + (month - 3)), year if month - 3 > 0 else year - 1)
period1 = get_start_date(4, 1, 2021)
period2 = get_end_date(5, 4, 2021)
url = prepare_url(ticker, period1, period2, 'weekly')
data = get_data_from_yahoo_finance(url)
dataframe = prepare_dataframe(data)
